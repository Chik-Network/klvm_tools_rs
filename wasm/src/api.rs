...
fn create_klvm_runner_run_failure(err: &RunFailure) -> JsValue {
    match err {
        RunFailure::RunErr(l, e) => create_klvm_runner_err(format!("{}: Error {}", l, e)),
        RunFailure::RunExn(l, e) => create_klvm_runner_err(format!("{}: Exn {}", l, e)),
    }
}

fn create_klvm_compile_failure(err: &CompileErr) -> JsValue {
    match err {
        CompileErr(l, e) => create_klvm_runner_err(format!("{}: Error {}", l, e)),
    }
}
