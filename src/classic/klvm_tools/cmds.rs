...
use crate::classic::klvm_tools::comp_input::{get_disassembly_ver, RunAndCompileInputData};
...
impl TConversion for OpcConversion {
    ...
    fn invoke(
        ...
        read_ir(hex_text)
            ...
            .unwrap_or_else(|err| Ok(t(allocator.null(), err))) // Original code printed error messages on stdout, ret 0 on KLVM error
    }
}
...
pub fn cldb_hierarchy(args: CldbHierarchyArgs) -> Vec<BTreeMap<String, YamlElement>> {
    let mut runner = HierarchiklRunner::new(
        args.runner,
        args.prim_map,
    ...
    loop {
        ...
        match runner.step() {
            ...
            Ok(HierarchiklStepResult::Info(Some(info))) => {
            ...
            Ok(HierarchiklStepResult::Info(None)) => {
                // Nothing
            }
            Ok(HierarchiklStepResult::Done(Some(info))) => {
            ...
            Ok(HierarchiklStepResult::Done(None)) => {
                // Nothing
            }
...
pub fn cldb(args: &[String]) {
    ...
    parser.add_argument(
        ...
        Argument::new()
            .set_action(TArgOptionAction::StoreTrue)
            .set_help("new style hierarchikl view of function calls and args".to_string()),
    );
    parser.add_argument(
        vec!["path_or_code".to_string()],
        Argument::new()
