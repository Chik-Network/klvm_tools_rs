...
use crate::classic::klvm_tools::stages::stage_0::{RunProgramOption, TRunProgram};
...
pub fn convert_to_klvm_rs(
    ...
    match head.borrow() {
        ...
        SExp::Cons(_, a, b) => convert_to_klvm_rs(allocator, a.clone()).and_then(|head_ptr| {
...
#[test]
fn test_convert_from_klvm_rs_00_byte() {
    let mut allocator = Allocator::new();
    let allocator_atom = allocator.new_atom(&[0]).expect("should convert");
    let srcloc = Srcloc::start("*test*");
    let result = convert_from_klvm_rs(&mut allocator, srcloc.clone(), allocator_atom)
    ...
    let node = convert_to_klvm_rs(&mut allocator, result).expect("should convert from mod");
    assert_eq!(allocator.atom(node), &[0]);
}

#[test]
fn test_convert_to_klvm_rs_m129() {
    let mut allocator = Allocator::new();
    let srcloc = Srcloc::start("*test*");
